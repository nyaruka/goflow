	...
	testCases := []struct {
	...
	}{
		{
			...
			[]string{
				...
				`bleu`,
				`fra`,
				`@(TITLE(run.results.favorite_color.category_localized)) it is! What is your favorite soda? (pepsi/coke)`,
				...
				`{ "contact": @(json(contact.uuid)), "soda": @(json(results.soda)) }`,
				`Great, you are done and like @results.soda! Webhook status was @results.webhook`,
