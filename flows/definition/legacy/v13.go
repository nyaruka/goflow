...
func newSendBroadcastAction(uuid uuids.UUID, text string, attachments []string, quickReplies []string, urns []urns.URN, contacts []*events.ContactReference, groups []*assets.GroupReference, legacyVars []string) migratedAction {
	d := map[string]any{
		"uuid": uuid,
		"type": "send_broadcast",
		"text": text,
...
func newStartSessionAction(uuid uuids.UUID, flow *assets.FlowReference, urns []urns.URN, contacts []*events.ContactReference, groups []*assets.GroupReference, legacyVars []string, createContact bool) migratedAction {
	d := map[string]any{
		"uuid": uuid,
		"type": "start_session",
		"flow": flow,
