...
func TestTemplateTranslation(t *testing.T) {
	...
	for i, tc := range tcs {
		tt := flows.NewTemplateTranslation(static.NewTemplateTranslation(channel, i18n.Locale("eng-US"), tc.Content, "a6a8863e_7879_4487_ad24_5e2ea429027c", map[string][]assets.TemplateParam{}))
...
func TestTemplate(t *testing.T) {
	...
	tt1 := static.NewTemplateTranslation(channel1Ref, i18n.Locale("eng"), "Hello {{1}}", "", map[string][]assets.TemplateParam{"body": {{Type: "text"}}})
	tt2 := static.NewTemplateTranslation(channel1Ref, i18n.Locale("spa-EC"), "Que tal {{1}}", "", map[string][]assets.TemplateParam{"body": {{Type: "text"}}})
	tt3 := static.NewTemplateTranslation(channel1Ref, i18n.Locale("spa-ES"), "Hola {{1}}", "", map[string][]assets.TemplateParam{"body": {{Type: "text"}}})
	tt4 := static.NewTemplateTranslation(channel2Ref, i18n.Locale("en"), "Hello {{1}}", "", map[string][]assets.TemplateParam{"body": {{Type: "text"}}})
