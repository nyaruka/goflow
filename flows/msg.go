...
type MsgTemplating struct {
	...
	Params_    map[string]ComponentVariables `json:"params,omitempty"`
}
...
func NewMsgTemplating(template *assets.TemplateReference, variables []string, namespace string) *MsgTemplating {
	...
	params := make(map[string]ComponentVariables)
	if len(variables) > 0 {
		params["body"] = ComponentVariables{Variables: variables}
	}
