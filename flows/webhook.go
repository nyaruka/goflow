...
var DefaultWebhookPayload = `{
	...
	"input": @(json(if(input, dict("attachments", input.attachments, "channel", input.channel, "created_on", input.created_on, "text", input.text, "type", input.type, "urn", if(input.urn, dict("display", format_urn(input.urn), "path", urn_parts(input.urn).path, "scheme", urn_parts(input.urn).scheme), null), "uuid", input.uuid), null))),
