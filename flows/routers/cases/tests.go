...
// classification is the shape of the NLU data that older flows may have stored in a result's
// extra field. It is preserved here so that has_intent / has_top_intent on legacy results can
// still be evaluated.
type classification struct {
	Intents []struct {
		Name       string          `json:"name"`
		Confidence decimal.Decimal `json:"confidence"`
	} `json:"intents,omitempty"`
...
func hasIntent(resultObj *types.XObject, name *types.XText, confidence *types.XNumber, topOnly bool) types.XValue {
	...
	c := &classification{}
	jsonx.Unmarshal(result.Extra, c)
	...
	for _, intent := range intents {
		...
		if intentName.Equals(name) && intent.Confidence.GreaterThanOrEqual(confidence.Native()) {
			extra := make(map[string]types.XValue, len(c.Entities))
			for entityName, possibilities := range c.Entities {
				if len(possibilities) > 0 {
					extra[entityName] = types.NewXText(possibilities[0].Value)
				}
			}
