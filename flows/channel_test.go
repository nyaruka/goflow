...
func TestChannelSetGetForURN(t *testing.T) {
	...
	assert.Nil(t, emptySet.GetForURN(flows.NewURN("tel", "+12345678999", "", nil), assets.ChannelRoleSend))

	// nil if not channel with correct role
	assert.Nil(t, receiverSet.GetForURN(flows.NewURN("tel", "+12345678999", "", receiver), assets.ChannelRoleSend))

	// can still match URN has a preferred channel with matching role
	assert.Equal(t, receiver, receiverSet.GetForURN(flows.NewURN("tel", "+12345678999", "", receiver), assets.ChannelRoleReceive))
	...
	assert.Equal(t, twitter, all.GetForURN(flows.NewURN("twitter", "nyaruka2", "", nil), assets.ChannelRoleSend))

	// if there's only one channel for that country, it's used
	assert.Equal(t, claro, all.GetForURN(flows.NewURN("tel", "+593971234567", "", nil), assets.ChannelRoleSend))

	// return nil for international send if the channels don't allow it
	assert.Nil(t, rwOnly.GetForURN(flows.NewURN("tel", "+593971234567", "", nil), assets.ChannelRoleSend))

	// but use them if they do
	assert.Equal(t, claro, all.GetForURN(flows.NewURN("tel", "+57971234567", "", nil), assets.ChannelRoleSend))

	// or if they're implicitly international by having no country
	assert.Equal(t, twilio, twOnly.GetForURN(flows.NewURN("tel", "+57971234567", "", nil), assets.ChannelRoleSend))

	// if there's multiple channels, one with longest number overlap wins
	assert.Equal(t, mtn, all.GetForURN(flows.NewURN("tel", "+250781234567", "", nil), assets.ChannelRoleSend))
	assert.Equal(t, tigo, all.GetForURN(flows.NewURN("tel", "+250721234567", "", nil), assets.ChannelRoleSend))

	// if there's no overlap, then last/newest channel wins
	assert.Equal(t, tigo, all.GetForURN(flows.NewURN("tel", "+250962222222", "", nil), assets.ChannelRoleSend))
	...
	assert.Equal(t, short1, all.GetForURN(flows.NewURN("tel", "+250781234567", "", nil), assets.ChannelRoleSend))
	assert.Equal(t, short1, all.GetForURN(flows.NewURN("tel", "+250771234567", "", nil), assets.ChannelRoleSend))
	assert.Equal(t, short2, all.GetForURN(flows.NewURN("tel", "+250721234567", "", nil), assets.ChannelRoleSend))
}
