...
func TestTriggerMarshaling(t *testing.T) {
	...
	ticket := flows.NewTicket("276c2e43-d6f9-4c36-8e54-b5af5039acf6", events.TicketStatusOpen, weather, user)
	...
	triggerTests := []struct {
		...
		{
			triggers.NewBuilder(flow).CallMissed(events.NewCallMissed(channel)).Build(),
			"call_missed",
		},
		{
		...
		{
			triggers.NewBuilder(flow).
				MsgReceived(events.NewMsgReceived(events.NewMsgIn(urns.URN("tel:+1234567890"), channel, "Hi there", nil, "SMS1234"), "")).
