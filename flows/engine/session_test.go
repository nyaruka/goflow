...
func TestRunResuming(t *testing.T) {
	...
	assert.Equal(t, events.RunStatusCompleted, session.Runs()[0].Status())
	assert.Equal(t, events.RunStatusCompleted, session.Runs()[1].Status())
	assert.Equal(t, events.RunStatusCompleted, session.Runs()[2].Status())
	...
	assert.Equal(t, events.RunStatusFailed, session.Runs()[0].Status())
	assert.Equal(t, events.RunStatusFailed, session.Runs()[1].Status())
}
...
func TestResumeAfterWaitWithMissingFlowAssets(t *testing.T) {
	...
	assert.Equal(t, events.RunStatusActive, session1.Runs()[0].Status())
	assert.Equal(t, events.RunStatusWaiting, session1.Runs()[1].Status())
	...
	assert.Equal(t, events.RunStatusFailed, session2.Runs()[0].Status())
	assert.Equal(t, events.RunStatusFailed, session2.Runs()[1].Status())
	...
	assert.Equal(t, events.RunStatusFailed, session3.Runs()[0].Status())
	assert.Equal(t, events.RunStatusCompleted, session3.Runs()[1].Status())
}
...
func TestSessionHistory(t *testing.T) {
	...
	assert.Equal(t, events.EmptyHistory, session1.History())
	...
	assert.Equal(t, &events.SessionHistory{
...
func TestMaxSprintsPerSession(t *testing.T) {
	...
	numResumes := 0
	for {
		msg := events.NewMsgIn("tel:+593979123456", nil, "Teal", nil, "SMS1234")
...
func TestEngineErrors(t *testing.T) {
	...
	_, err = session.Resume(ctx, resumes.NewDial(events.NewDialEnded(events.NewDial(events.DialStatusAnswered, 10))))
