...
func generateFunctionListing(baseDir, outputDir, localesDir string, items map[string][]*TaggedItem) error {
	funcItems := items["function"]
	listings := make([]*functionListing, len(funcItems))

	poLib := i18n.NewLibrary(localesDir, srcLanguage)
	...
	if err := poLib.Update("functions", pot); err != nil {
		return err
	}
