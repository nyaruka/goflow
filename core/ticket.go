...
// TicketEnvelope is the serialized form of a ticket. Use its Unmarshal method to resolve it
// against assets into a live ticket.
...
// Unmarshal resolves this envelope into a live ticket. If the topic or assigned user can't
// be found in the assets, we report the missing asset and return ticket without those.
func (e *TicketEnvelope) Unmarshal(topics *TopicAssets, users *UserAssets, missing assets.MissingCallback) *Ticket {
	if e.Status == "" {
	...
	var assignee *User
	if e.Assignee != nil {
		assignee = users.Get(e.Assignee.UUID)
		if assignee == nil {
