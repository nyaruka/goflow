...
func HasState(env utils.Environment, args ...interface{}) interface{} {
	...
	states, err := utils.FindLocations(env, text, utils.LocationLevel(1), nil)
	if err != nil {
		return err
	}
	if len(states) > 0 {
...
func HasDistrict(env utils.Environment, args ...interface{}) interface{} {
	...
	states, err := utils.FindLocations(env, stateText, utils.LocationLevel(1), nil)
	if err != nil {
		return err
	}
	if len(states) > 0 {
		districts, err := utils.FindLocations(env, text, utils.LocationLevel(2), states[0])
		if err != nil {
			return err
		}
		if len(districts) > 0 {
	...
	if stateText == "" {
		districts, err := utils.FindLocations(env, text, utils.LocationLevel(2), nil)
		if err != nil {
			return err
		}
		if len(districts) == 1 {
...
func HasWard(env utils.Environment, args ...interface{}) interface{} {
	...
	if len(states) > 0 {
		districts, err := utils.FindLocations(env, districtText, utils.LocationLevel(2), states[0])
		if err != nil {
			return err
		}
		if len(districts) > 0 {
			wards, err := utils.FindLocations(env, text, utils.LocationLevel(3), districts[0])
			if err != nil {
				return err
			}
			if len(wards) > 0 {
	...
	if districtText == "" {
		wards, err := utils.FindLocations(env, text, utils.LocationLevel(3), nil)
		if err != nil {
			return err
		}
		if len(wards) == 1 {
