...
func TestXFunction(t *testing.T) {
	...
	assert.Equal(t, types.NewXNumberFromInt(0), func1.Call(t.Context(), env, nil))
	assert.Equal(t, types.NewXNumberFromInt(2), func1.Call(t.Context(), env, []types.XValue{types.NewXText("a"), types.NewXText("b")}))
	...
	assert.Equal(t, types.NewXText("c"), anon1.Call(t.Context(), env, nil))
