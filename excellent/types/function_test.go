...
func TestXFunction(t *testing.T) {
	...
	func1 := types.NewXFunction("foo", func(ctx context.Context, env envs.Environment, args ...types.XValue) types.XValue {
		return types.NewXNumberFromInt(len(args))
	})
	func2 := types.NewXFunction("bad", func(ctx context.Context, env envs.Environment, args ...types.XValue) types.XValue {
		return types.NewXErrorf("boom")
	})
	anon1 := types.NewXFunction("", func(ctx context.Context, env envs.Environment, args ...types.XValue) types.XValue {
		return types.NewXText("c")
	})
	anon2 := types.NewXFunction("", func(ctx context.Context, env envs.Environment, args ...types.XValue) types.XValue {
		return types.NewXText("d")
	})
	...
	assert.Equal(t, types.NewXNumberFromInt(0), func1.Call(context.Background(), env, nil))
	assert.Equal(t, types.NewXNumberFromInt(2), func1.Call(context.Background(), env, []types.XValue{types.NewXText("a"), types.NewXText("b")}))

	assert.Equal(t, types.NewXErrorf("error calling bad(...): boom"), func2.Call(context.Background(), env, nil))
	...
	assert.Equal(t, types.NewXText("c"), anon1.Call(context.Background(), env, nil))
...
func TestToXFunction(t *testing.T) {
	foo := types.NewXFunction("foo", func(ctx context.Context, env envs.Environment, args ...types.XValue) types.XValue {
		return types.NewXText("foo")
	})
	anon := types.NewXFunction("", func(ctx context.Context, env envs.Environment, args ...types.XValue) types.XValue {
