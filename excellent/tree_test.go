...
func TestExpressionsToString(t *testing.T) {
	...
	one := &NumberLiteral{val: decimal.RequireFromString(`1`)}
	two := &NumberLiteral{val: decimal.RequireFromString(`2`)}
	...
	assert.Equal(t, `123.5`, (&NumberLiteral{val: decimal.RequireFromString(`123.5`)}).String())
	assert.Equal(t, `123`, (&NumberLiteral{val: decimal.RequireFromString(`123.0`)}).String())
