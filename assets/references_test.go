...
func TestReferences(t *testing.T) {
	...
	modelRef := assets.NewModelReference("61602f3e-f603-4c70-8a8f-c477505bf4bf", "GPT-4")
	assert.Equal(t, "llm", modelRef.Type())
	assert.Equal(t, "61602f3e-f603-4c70-8a8f-c477505bf4bf", modelRef.Identity())
	assert.Equal(t, uuids.UUID("61602f3e-f603-4c70-8a8f-c477505bf4bf"), modelRef.GenericUUID())
	assert.Equal(t, "llm[uuid=61602f3e-f603-4c70-8a8f-c477505bf4bf,name=GPT-4]", modelRef.String())
	assert.False(t, modelRef.Variable())
	assert.NoError(t, utils.Validate(modelRef))

	// model references must always be concrete
	assert.EqualError(t, utils.Validate(assets.NewModelReference("", "GPT-4")), "field 'uuid' is required")
