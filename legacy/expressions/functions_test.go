...
func TestMigrateFunctionCall(t *testing.T) {
	var tests = []struct {
		...
		{old: `@(FIELD(flow.favorite_color, child.age, ","))`, new: `@(field(results.favorite_color, child.results.age.value - 1, ","))`, val: ``},
		...
		{old: `@(FIXED(contact.age, 2, false))`, new: `@(format_number(fields.age, 2, false))`, val: `23.00`},
		{old: `@(FIXED(contact.age, 2))`, new: `@(format_number(fields.age, 2))`, val: `23.00`},
		{old: `@(FIXED(contact.age))`, new: `@(format_number(fields.age))`, val: `23.00`},
		...
		{old: `@(LEN(contact.first_name))`, new: `@(length(word(contact.name, 0)))`, val: `4`},
		{old: `@(LOWER(contact.first_name))`, new: `@(lower(word(contact.name, 0)))`, val: `ryan`},
		...
		{old: `@(WORD(flow.favorite_color, child.age - 22))`, new: `@(word(results.favorite_color, legacy_add(child.results.age.value, -22) - 1))`},
