...
func TestMigrateFunctionCall(t *testing.T) {
	var tests = []struct {
		...
		{old: `@(AND(contact.age > 30, flow.amount < 5))`, new: `@(and(contact.fields.age > 30, results.amount < 5))`, val: `false`},
		{old: `@(AND(contact.gender = "F", contact.age >= 18))`, new: `@(and(contact.fields.gender = "F", contact.fields.age >= 18))`, val: `false`},
		...
		{old: `@(FIXED(contact.age, 2, false))`, new: `@(format_number(contact.fields.age, 2, false))`, val: `23.00`},
		{old: `@(FIXED(contact.age, 2))`, new: `@(format_number(contact.fields.age, 2))`, val: `23.00`},
		{old: `@(FIXED(contact.age))`, new: `@(format_number(contact.fields.age))`, val: `23.00`},
		{old: `@(HOUR(NOW()))`, new: `@(format_datetime(now(), "tt"))`},
		{old: `@(IF(contact.gender = "M", "Sir", "Madam"))`, new: `@(if(contact.fields.gender = "M", "Sir", "Madam"))`, val: `Madam`},
		{old: `@(INT(contact.age))`, new: `@(round_down(contact.fields.age))`, val: `23`},
