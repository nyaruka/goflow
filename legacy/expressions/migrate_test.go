...
func TestMigrateTemplate(t *testing.T) {
	var tests = []testTemplate{
		...
		//{old: `@(WORD_SLICE(flow.favorite_color, 2, 4, TRUE))`, new: `@(word_slice(run.results.favorite_color, 2 - 1, 4 - 1, " "))`},
		...
		//{old: `@(WORD(flow.favorite_color, 1, TRUE))`, new: `@(word(run.results.favorite_color, 1 - 1, " "))`},
