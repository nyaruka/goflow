...
func init() {
	...
	mappings = []mapping{
		...
		{re(`^(?:(?:flow|step)\.)?((?:parent|child)\.)?contact\.tel_e164$`), `default(urn_parts(${1}urns.tel).path, "")`},
		{re(`^(?:(?:flow|step)\.)?((?:parent|child)\.)?contact\.tel$`), `format_urn(${1}urns.tel)`},
		{re(`^(?:(?:flow|step)\.)?((?:parent|child)\.)?contact\.(` + schemesRe + `)$`), `default(urn_parts(${1}urns.$2).path, "")`},
