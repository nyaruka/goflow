...
func TestAirtimeService(t *testing.T) {
	...
	err = svc.Confirm(ctx, transfer, func(*core.HTTPLog) {})
	assert.NoError(t, err)

	// recipients containing "666" cause Create to fail (used by other tests)
	_, err = svc.Create(ctx, uuid, urns.NilURN, urns.URN("tel:+1666"), map[string]decimal.Decimal{"USD": decimal.RequireFromString("3")}, func(*core.HTTPLog) {})
